...
from Tana.ministries.forms import MinistriesForm
...
@ministry_bp.route('/ministries/add', methods=['GET', 'POST'])
def add_ministry():
    form = MinistriesForm()
    if form.validate_on_submit():
        ...
        ministry = Ministries(
            ...
            documents_presented=form.documents_presented.data,
            date_documents_presented=form.date_documents_presented.data
        )
        # Add and save the new ministry
...
def edit_ministry(id):
    ...
    form = MinistriesForm(obj=ministry)
